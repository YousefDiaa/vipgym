...
export default function RulesSection() {
  ...
  return (
    <section id="rules" className="py-20 bg-stone-900/40 relative overflow-hidden">
      {/* Decorative Blur */}
      <div className="absolute top-0 right-1/4 w-96 h-96 bg-gold-400/5 rounded-full blur-3xl" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-3xl sm:text-4xl font-display font-black text-white mb-4">
            ميثاق العضوية: <span className="gold-gradient-text">الحقوق والالتزامات</span>
          </h2>
          <p className="text-stone-300 text-base sm:text-lg font-sans">
            نهتم بتوفير بيئة تمرين مثالية وراقية لجميع الأعضاء، ونعمل بتوازن تام لضمان جودة وسير الخدمة مع حفظ حقوق والتزامات الجميع.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-10">
          <button
            ...
            className={`px-5 py-3 rounded-xl font-bold text-sm sm:text-base transition-all flex items-center gap-2 border ${
              activeTab === "rights"
        ...
        <div className="bg-red-500/10 border border-red-500/30 p-5 rounded-2xl mb-8 flex flex-col sm:flex-row gap-4 items-center">
        ...
        {/* Active Content Grid */}
        <div className="glass-card rounded-3xl border border-stone-800 p-6 sm:p-8">
            ...
            {activeTab === "rights" && (
              ...
              >
                <div className="flex items-center gap-3 border-b border-stone-800 pb-4">
                  <div className="p-2.5 bg-gold-400/10 text-gold-400 rounded-lg">
                    <HeartHandshake className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-lg font-display font-black text-white">ضمان الحقوق والإرشاد العام</h3>
                    <p className="text-stone-400 text-xs">شعارنا دائمًا: نحن في خدمتك، ثق بنا.</p>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6 pt-2">
                  <ul className="space-y-4">
                    {generalGuidelines.slice(0, 3).map((guide, idx) => (
                      <li key={idx} className="flex gap-3 bg-stone-950/60 p-4 rounded-xl border border-stone-800/80">
                        <span className="w-6 h-6 rounded-full bg-gold-400/10 text-gold-400 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                          {idx + 1}
                        </span>
                  ...
                  <ul className="space-y-4">
                    ...
                    <div className="bg-gold-500/10 border border-gold-500/20 p-4 rounded-xl flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <a href="https://wa.me/201009244078" className="text-sm font-black text-gold-400 hover:underline">
                          01009244078
                        </a>
                      </div>
                    </div>
              ...
              >
                <div className="flex items-center gap-3 border-b border-stone-800 pb-4">
                  ...
                  <div>
                    ...
                    <p className="text-stone-400 text-xs">التزامك يحفظ النظام، والنظافة، وسلامة الأوزان والأجهزة للجميع.</p>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4 pt-2">
                  {obligationsBehavioral.map((item, idx) => (
                    <div
                      key={idx}
                      className={`p-3.5 rounded-xl border flex gap-3 text-sm font-sans items-start ${
                        item.includes("🚫")
                          ? "bg-red-500/10 border-red-500/20 text-stone-200"
                          : "bg-stone-950/40 border-stone-800/80 hover:border-stone-700/80 transition-all text-stone-300"
                      }`}
                    >
              ...
              >
                <div className="flex items-center gap-3 border-b border-stone-800 pb-4">
                  ...
                  <div>
                    ...
                    <p className="text-stone-400 text-xs">إجراءات رسمية تضمن حقوق النادي وحقك المالي فور التسجيل.</p>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4 pt-2">
                  {obligationsFinancial.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className={`p-4 rounded-xl border flex gap-3.5 items-start text-sm font-sans ${
                          isCrucial
                            ? "bg-gold-500/5 border-gold-500/20 text-stone-200 shadow-[0_0_15px_rgba(212,175,55,0.03)]"
                            : "bg-stone-950/40 border-stone-800/80 text-stone-300 hover:border-stone-700 transition-all"
                        }`}
                      >
                        <span className={`w-5.5 h-5.5 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 mt-0.5 ${
                          isCrucial ? "bg-gold-500 text-black font-black" : "bg-stone-900 border border-stone-800 text-stone-400"
                        }`}>
                          {idx + 1}
                        </span>
