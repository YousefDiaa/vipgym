...
export default function FreeServicesList() {
  ...
  return (
    <section id="freebies" className="py-20 bg-stone-950 relative overflow-hidden">
      {/* Decorative Blur */}
      <div className="absolute top-1/4 right-0 w-96 h-96 bg-gold-400/5 rounded-full blur-3xl" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-xs uppercase tracking-widest text-gold-400 font-bold font-mono">هدايا ومزايا إضافية</span>
          <h2 className="text-3xl sm:text-4xl font-display font-black text-white mt-2 mb-4">
            الخدمات المجانية <span className="gold-gradient-text">لأعضاء الجيم</span>
          </h2>
          ...
          {freeServices.map((service, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`p-6 sm:p-8 rounded-3xl border transition-all duration-300 relative overflow-hidden flex flex-col justify-between group hover:scale-[1.01] ${
                  isPrivateOnly
                    ? "bg-purple-950/20 border-purple-500/35 hover:border-purple-500/50"
                    : "glass-card border-stone-800 hover:border-gold-500/30"
                }`}
              >
                {/* Free Badge */}
                <span className={`absolute top-4 left-4 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider ${
                  isPrivateOnly
                ...
                <div>
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 border ${
                    isPrivateOnly
                      ...
                      : "bg-stone-900 border-stone-800 text-gold-400 group-hover:border-gold-500/40 transition-all"
                  }`}>
